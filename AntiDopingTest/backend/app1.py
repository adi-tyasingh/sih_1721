...
def fetch_athlete_data(url):
    ...
    if honours_container:
        for honour_div in honours_container.find_all('div', class_='athletesBio_athletesBioHonour__1Zqdq'):
            ...
            title_div = honour_div.find('div', class_='athletesBio_athletesBioHonourValue__QHLA_')
...
@app.route('/athlete-data/<country>/<athlete_slug>', methods=['GET'])
def athlete_data(country ,athlete_slug):
    # Construct the URL using the athlete_slug directly from the URL
    url = f"https://worldathletics.org/athletes/{country}/{athlete_slug}"
...
def generate_search_competitors_payload(query=None, gender=None, discipline_code=None, environment=None, country_code=None):
    return {
...
def search_competitors():
    ...
    payload = generate_search_competitors_payload(query=query,country_code=country,gender=gender)
    ...
    try:
        ...
        response_data = json.loads(response.text)  # Ensure this is parsed as JSON
